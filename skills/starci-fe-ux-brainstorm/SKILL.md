...
description: >
  ...
  Step 1 of the self-learning loop. It READS the house's decision memory — `journals/mindset/<block>.md`
  (why we chose & shaped each block), `journals/ux/CONTENT.md` (page-level UX decisions + UX lessons),
...
skill-relative paths are `../../journals`, `../../cannon`. This skill **does NOT write code** — it reads,
fetches, proposes, draws widgets.

## 1. Read the house knowledge (it grows every loop)
- **`../../journals/mindset/<block>.md`** — WHY we chose & shaped each block (card, sidebar, tabs, chip,
  stat…). Open the ones relevant to the page; `../../journals/mindset/CONTENT.md` is the index. **Reuse the
  house's logic — don't re-decide.**
- **`../../journals/ux/CONTENT.md`** — page-level UX decisions + UX pattern lessons already captured.
...
`../../journals/ux/CONTENT.md`: Coursera, Udemy, Duolingo, Khan, Brilliant, Codecademy, edX…). Lift what they
do well; note what NOT to copy. (Apply will append the durable takeaway to `journals/ux/` at the end.)
...
Each direction: anchored to a **mindset decision** + a **fetched ref** + a design-system archetype.
...
> No separate memory or refs folders. The knowledge IS `journals/mindset/` + `journals/ux/` + `cannon/` —
> they get richer every loop, so each brainstorm starts smarter than the last.
